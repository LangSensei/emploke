...
export interface Runtime {
  ...
  /**
   * Optional. Read runtime-managed display metadata for a task —
   * principally the `title` field that the runtime's underlying
   * CLI generates from the first user prompt (Copilot's
   * `workspace.yaml.name`, etc.). Called by `TaskManager.list` /
   * `get` to enrich the persisted task with a human-friendly
   * label that's shorter and more scannable than the raw
   * `instructions` string.
   *
   * `opts.metadata` is the task's open-shape metadata bag (same
   * shape as {@link taskActivity} / {@link deleteTaskState}); the
   * runtime reads its own keys (typically `runtimeSessionId`) to
   * locate the source of truth.
   *
   * Returns `null` when no metadata is available (task hasn't
   * started, runtime hasn't generated a title, runtime doesn't
   * support per-task display metadata). Manager treats null as
   * "fall through to the next display source" (typically the
   * first non-empty line of `task.instructions`).
   *
   * Why a separate hook from {@link taskActivity}: title is
   * cheap (single small file read) and called frequently (every
   * task-list poll); activity is potentially expensive and called
   * only on detail-page open. Keeping them separate lets the
   * manager batch + cache differently.
   */
  taskMetadata?(opts: TaskMetadataOpts): Promise<TaskRuntimeDisplayMetadata | null>;
}
...
/** Inputs to {@link Runtime.taskMetadata}. */
export interface TaskMetadataOpts {
  /**
   * The task's open-shape metadata bag (`Task.metadata`). The runtime
   * looks up its own keys (typically `runtimeSessionId`).
   */
  readonly metadata: Readonly<Record<string, unknown>>;
}

/**
 * Runtime-supplied display metadata for a task. Populated by
 * {@link Runtime.taskMetadata} from the runtime's own state
 * (Copilot reads `workspace.yaml`'s `name` / `summary` / `user_named`
 * fields). Surfaced through `Task.metadata.title` /
 * `Task.metadata.userTitled` after the manager merges.
 */
export interface TaskRuntimeDisplayMetadata {
  /**
   ...
   * runtime hasn't produced one yet (task hasn't received its first
   * turn) or when the source field is empty.
   */
  readonly title: string | null;
  /**
   * True when the user has explicitly renamed the entity via the
   * runtime CLI's rename command (Copilot's `user_named: true`).
   * Consumers MUST NOT overwrite the title in this case even if
   * they regenerate one — user intent wins.
   */
  readonly userTitled: boolean;
  /**
   * Last-active ISO timestamp from the runtime's own clock. Lets
   * the manager surface "when did this task last write something"
   * separately from the persisted `endedAt`.
   */
