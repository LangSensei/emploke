/**
 * Base class for every domain value object.
 *
 * Mirrors eShop's `ValueObject` (Ordering.Domain/SeedWork/ValueObject.cs).
 * A value object has **no identity** — equality is purely structural,
 * computed from a tuple of "equality components" the subclass declares.
 * Two `WorkspaceId(`abc`)` instances are equal even though they are
 * different object references; cf. {@link Entity}, where equality is
 * by id.
 *
 * ## Subclass contract
 *
...
export abstract class ValueObject {
   ...
   * positionally. Components that are themselves VOs are compared via
   * their own `equals` (not `===`).
   */
