 ...
 * em.transactional. Each bounded context owns its own EM, so this
 * behaviour belongs to @emploke/workspace specifically. Domain-event
 * dispatch happens via DomainEventDispatcher (MikroORM beforeFlush
 * subscriber registered in bootstrap), so this behaviour stays
 * single-purpose: BEGIN / COMMIT / ROLLBACK only. em.transactional
 * auto-flushes at the end of the callback, which triggers the
 * subscriber's beforeFlush hook (events dispatched, then SQL writes).
 *
