...
describe("Skill.create", () => {
  ...
  it("computes a stable canonical SHA-256 of the frontmatter (NOT the body)", () => {
    const s = Skill.create(MIN_VALID, "file:/abs/x", "test");
    expect(s.frontmatterSha256).toMatch(/^[0-9a-f]{64}$/);
    // deterministic
    expect(s.frontmatterSha256).toBe(s.frontmatterSha256);
  });

  it("body-only change does NOT change the frontmatter SHA", () => {
    // Author contract: body changes WITHOUT a version bump are the
    // author's bug, not emploke's. SHA reflects identity-bearing
    // metadata only.
    const s1 = Skill.create(MIN_VALID, "file:/abs/x", "test");
    const s2 = Skill.create(MIN_VALID + "\nextra body content", "file:/abs/x", "test");
    expect(s2.frontmatterSha256).toBe(s1.frontmatterSha256);
  });

  it("version change DOES change the frontmatter SHA", () => {
    const s1 = Skill.create(MIN_VALID, "file:/abs/x", "test");
    const bumped = MIN_VALID.replace("version: 1.0.0", "version: 2.0.0");
    const s2 = Skill.create(bumped, "file:/abs/x", "test");
    expect(s2.frontmatterSha256).not.toBe(s1.frontmatterSha256);
  });

  it("description change DOES change the frontmatter SHA", () => {
    const s1 = Skill.create(MIN_VALID, "file:/abs/x", "test");
    const updated = MIN_VALID.replace("description: Helpful patterns", "description: Updated");
    const s2 = Skill.create(updated, "file:/abs/x", "test");
    expect(s2.frontmatterSha256).not.toBe(s1.frontmatterSha256);
  });

  it("dependency addition DOES change the frontmatter SHA", () => {
    const s1 = Skill.create(MIN_VALID, "file:/abs/x", "test");
    const withDep = MIN_VALID.replace(
      "version: 1.0.0",
      `version: 1.0.0
dependencies:
  skills:
    ...
    const s2 = Skill.create(withDep, "file:/abs/x", "test");
    expect(s2.frontmatterSha256).not.toBe(s1.frontmatterSha256);
  });

  it("frontmatter SHA is canonical (key order independent)", () => {
    // js-yaml object key order in input shouldn't affect hash
    const order1 = `---
name: tool-use
description: x
version: 1.0.0
...
# Body
`;
    const s1 = Skill.create(order1, "file:/abs/x", "test");
    const s2 = Skill.create(order2, "file:/abs/x", "test");
    expect(s2.frontmatterSha256).toBe(s1.frontmatterSha256);
  });
