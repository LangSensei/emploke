...
async function makeSkill(name: string, deps?: { skills?: string[]; mcps?: string[] }): Promise<string> {
  ...
  const lines = [
    ...
    ...(deps ? [`dependencies:`, ...(deps.skills ? [`  skills:`, ...deps.skills.map(s => `    - ${s}`)] : []), ...(deps.mcps ? [`  mcps:`, ...deps.mcps.map(m => `    - ${m}`)] : [])] : []),
    "---",
  ].join("\n");
...
async function makeAgent(name: string, deps?: { skills?: string[]; mcps?: string[] }): Promise<string> {
...
describe("Resolver", () => {
  ...
  it("resolves skill with direct deps", async () => {
    ...
    await skills.install(await makeSkill("security-audit", { skills: ["cve-db"], mcps: ["semgrep"] }));
