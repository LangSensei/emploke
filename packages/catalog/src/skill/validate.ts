...
/**
 * Per-kind name/scope/FQN validators for skills. Thin shadow over
 * `_shared/validate-shared.ts` — see that file for the grammar.
 * MCP names have a different grammar (see `mcp/validate.ts`).
 */
const validators: AnchoredValidators = makeValidators(SkillNameInvalidError, {
  short: ["tool-use", "web-search"],
  fqn: "public/tool-use",
});
