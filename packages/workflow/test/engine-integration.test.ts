 ...
 *
 * The test escape hatch (`trustedCallerForTesting: true`, spec #325
 * D7 Option A) is used here to add worker nodes directly without
 * standing up a coord runner. The fake coord runner is a passthrough
 * stub whose `dispatch` immediately fires `onTerminal({succeeded})`
 * so workflow lifecycle assertions can land cleanly.
 *
