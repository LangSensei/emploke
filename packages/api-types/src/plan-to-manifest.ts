 ...
 * the install + sync flows. Lives in `@emploke/api-types` so both the
 * server's `planToManifest` projection AND the dashboard / CLI clients
 * can typecheck against the same shape without one package
