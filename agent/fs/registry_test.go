...
import (
	...
	regfs "github.com/LangSensei/emploke/agent/fs"
)
...
func TestResolve_RecursiveDependencies(t *testing.T) {
	...
	expected := []string{"skill:xiaohongshu", "mcp:playwright", "skill:planning"}
...
`), 0644)
	...
	if len(missing) != 1 || missing[0] != "skill:nonexistent-skill" {
		t.Fatalf("expected [skill:nonexistent-skill], got %v", missing)
	}
}

func TestInstallAndRemoveAgent(t *testing.T) {
	reg, _ := setupTestRegistry(t)
	ctx := context.Background()

	// Create a source agent directory
	srcDir := t.TempDir()
	os.WriteFile(filepath.Join(srcDir, "AGENT.md"), []byte(`---
name: new-agent
version: 1.0.0
type: agent
capabilities:
...
`), 0644)
	...
	if err := reg.InstallAgent(ctx, "new-agent", srcDir); err != nil {
		t.Fatal(err)
	}
...
`), 0644)
	...
	if !names["skill:unused"] {
		t.Errorf("expected unused to be orphan, got %v", names)
	}
}

func TestInstallAndGetCapability_Skill(t *testing.T) {
	reg, _ := setupTestRegistry(t)
	ctx := context.Background()
...
`), 0644)
	...
	if err := reg.Install(ctx, "new-skill", srcDir); err != nil {
		t.Fatal(err)
	}
...
func TestInstallAndGetCapability_MCP(t *testing.T) {
	reg, _ := setupTestRegistry(t)
	ctx := context.Background()

	// Create source MCP json file
	srcFile := filepath.Join(t.TempDir(), "new-mcp.json")
	os.WriteFile(srcFile, []byte(`{"type":"stdio","command":"node","args":["server.js"]}`), 0644)

	if err := reg.Install(ctx, "new-mcp", srcFile); err != nil {
		t.Fatal(err)
	}
...
func TestDependents(t *testing.T) {
	...
	deps, err := reg.Dependents(ctx, "skill:xiaohongshu")
	if err != nil {
		t.Fatal(err)
	}
	...
	deps, err = reg.Dependents(ctx, "skill:planning")
	if err != nil {
		t.Fatal(err)
	}
	if len(deps) != 0 {
...
func TestInstallAgent_Idempotent(t *testing.T) {
	reg, _ := setupTestRegistry(t)
	ctx := context.Background()

	// Create source with different version
	srcDir := t.TempDir()
	os.WriteFile(filepath.Join(srcDir, "AGENT.md"), []byte(`---
name: researcher
version: 2.0.0
...
`), 0644)
	...
	if err := reg.InstallAgent(ctx, "researcher", srcDir); err != nil {
		t.Fatal(err)
	}
